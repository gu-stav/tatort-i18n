...
var translator = {},
    generateFileNameByUrl = function( url, extension ) {
      ...
      if( !fileName || !fileName[ 1 ] ) {
        throw new Error( 'Either the URL Format has changed, or you ' +
                         'pasted an invalid URL. Make sure you copied the ' +
                         'URL of the page, where the Video can be actually ' +
                         'played ... (URL: ' + url + ')' );
      }

      /* just to make the filename a bit nicer to read */
      fileName = fileName[ 1 ]
                  .replace( /-video-tgl-[a-z]+-20-uhr-100/gi, '' );
    ...
    getVideoFilePathByUrl = function( url ) {
      return __dirname + '/../video/' + generateFileNameByUrl( url, 'mp4' );
    };
  ...
  var videoAlreadyExists = function() {
      ...
      getVideoUrl = function( url ) {
        var generatePlayerConfigUrl = function( url ) {
              return url.replace( '100.html', '100~playerXml.xml' )[ 1 ];
            };

        var fileName = generateFileNameByUrl( url, 'mp4' ),
            playerUrl = generatePlayerConfigUrl( url ),
            videoUrl = '',
            videoRegex = /<something>([a-zA-Z0-9]+\.mp4)</something>$/;
      ...
      downloadVideo = function( url, fileName ) {
        var savedFilePath = getVideoDirectory() + fileName,
            savedFile = fs.createWriteStream( savedFilePath ),
      ...
      translate = function( text ) {
        return new Promise(function( resolve, reject ) {
          if( !config[ 'translate-api-key' ] ) {
            return reject( new Error( 'No API Key in config.json provided' ) );
          }
